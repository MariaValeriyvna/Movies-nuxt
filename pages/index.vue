  ...
  methods: {
    async getMovies() {
      const data = axios.get(
          'https://api.themoviedb.org/3/movie/now_playing?api_key=5f0597a602c883ca40d0dc7c4f3cfab5&language=en-US&page=1'
      )
      const result = await data
      const dataMovies: [] = result.data.results
    ...
    async getSearchMovies() {
      const data = axios.get(
          `https://api.themoviedb.org/3/search/movie?api_key=5f0597a602c883ca40d0dc7c4f3cfab5&language=en-US&page=1&query=${this.searchInput}`
      )
      const result = await data
      const dataMovies: [] = result.data.results
      dataMovies.forEach((movie) => {
